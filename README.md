# Homework 1: `git` and `make` and `gcc`

## Goals

This homework serves several purposes:

1. Get you familiar with the homework assignment submission system, which uses `git`
...
In particular, we will be building and testing your code on `ecegrid` machines, which you can `ssh` to using the following command
(replace USERNAME with your Purdue user name):

```
> ssh USERNAME@ecegrid.ecn.purdue.edu
```

In the rare case where the default ecegrid node is not responsive, you can use one of the following commands to switch to another node:

```
> ssh USERNAME@ecegrid-thin1.ecn.purdue.edu
> ssh USERNAME@ecegrid-thin2.ecn.purdue.edu
> ssh USERNAME@ecegrid-thin3.ecn.purdue.edu
> ssh USERNAME@ecegrid-thin4.ecn.purdue.edu
```
...
> Nevertheless, you are responsible for **making sure that your code runs properly in `ecegrid` (which is our test environment).**
...
Our homework submission system is built around [GitHub Classroom](https://classroom.github.com). What this means is that your homeworks will be distributed as *git repositories*, and you will submit your homework by pushing your code to GitHub. Part of the goal of this homework is to familiarize yourself with this process.
...
Once you have a GitHub username, let us know what it is so we can link your submissions to your Purdue account. To do so, fill out [this form](https://forms.gle/g88uGhQHQYEWFaPm8). **If you do not fill out this form, we will not be able to assign your grades properly.**

Then, create your HW1 repository. To do this, click on the link provided in Brightspace. This will set up a repository containing this README as well as starter code for HW1.
...
```
> git clone git@github.com:Purdue-ECE264-001-Fall2022-Wang/hw1-USERNAME.git
```
...
We will grade whichever version has the `final_ver` tag at the due date. **Please make sure to name the submitted version as `final_ver`.
Submissions with any other version names such as (`Final_ver`, `finalver`, `final_version`, etc.) will not be graded.**
